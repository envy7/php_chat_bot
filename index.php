...
$access_token = "EAASK50x8ys8BALLQohOLMNhZAkUroYTmMQWmPSWBCS302uAG3T6WqhULxiQSxDdQZAmgNtn0KHllE0eCDJ0Mk9BXEI35QGAwzX7aEZAFv5rQ91b1nygFs8fD0RW2eOHm3Y2S0ZARGPUWBUNzS3AZBKT8JByFwTumnx85geoQ8agZDZD";
...
if($sender != 217358738681243 && isset($message)){
    ...
    $message_to_reply = getapi_ai_call($message,$sender);
...
function checkupdatestatus($url){
    ...
    if(!(($row_meta_data[0] == $date) && ($row_meta_data[1] == "Y"))){
            ...
            for($i=0;$i<$num_query;$i++){
                ...
                updateInterests($row[0],$row[1],$url);
            }
            ...
            for($i=0;$i<$num_query;$i++){
                ...
                $schedule = $row[1]." today";
                updateSchedule($row[0],$schedule,$url);
            }
...
function secondtime($sender,$url,$message){
    $db = $GLOBALS['db'];
    add_user_details($sender, $message,2);
...
****************/
function updateInterests($sender,$interest,$url){

                send_simple_message_messenger($sender,"News for Your Interests today", $url);

                $interestArray = getInterestArray($interest);
                getNewsInterests($interestArray,$url,$sender);
}

function updateSchedule($sender,$schedule,$url){

               send_simple_message_messenger($sender,"Your Schedules for Today", $url);

                send_simple_message_messenger($sender,$schedule,$url);
}
...
function getInterestArray($string){
    
    $GLOBALS['count']=preg_match_all ("/\,[a-zA-Z]*\,|^[a-zA-Z]*\,|[a-zA-Z]*\,|[a-zA-Z]*$/", $string, $tagarray);
    return $tagarray;
}
...
function add_interest_db($sender, $message,$num){
    $db = $GLOBALS['db'];
...
function add_user_details($sender, $message,$num){
        $db = $GLOBALS['db'];
        $sql = "UPDATE `clients` SET `replies`='$num' WHERE `id` = '$sender'";
    mysqli_query($db,$sql);
...
Send message to Facebook Methods
...
function getapi_ai_call($message,$sender){
...
function getNewsInterests($interestArray,$url,$sender){
    ...
    while($j< $count){
       ...
       $hashtag = str_replace(',','',$interestArray[0][$j]);
       $newsquery = urlencode($hashtag);
       $chnews = curl_init();
       curl_setopt_array($chnews, array(
        CURLOPT_RETURNTRANSFER => 1,
       ...
       curl_close($chnews);
       $resp_news = json_decode($resp, true);
       $news_title[$j] = $resp_news['value'][0]['name'];
       $news_url[$j] = $resp_news['value'][0]['url'];
       $news_image_url[$j] = $resp_news['value'][0]['image']['thumbnail']['contentUrl'];
       $news_desc[$j] = $resp_news['value'][0]['description'];  
       $j++;
   }
